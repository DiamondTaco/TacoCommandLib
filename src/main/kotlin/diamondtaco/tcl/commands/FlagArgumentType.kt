...
interface Parser<T> : Serializable, ArgumentType<T> {
    ...
    fun getCompletions(context: CommandContext<ServerCommandSource>): List<String>
    ...
    override fun <S : Any?> listSuggestions(
        ...
        val completions = getCompletions(context!! as CommandContext<ServerCommandSource>)
        return completions.fold(builder!!) { b, s -> b.suggest(s) }.buildFuture()
    }
}

fun String.toMojangReader() = StringReader(this)

data class Marshal<T>(val parser: Parser<T>) : Parser<T> by parser
...
class MarshalArgumentType<T>(val parser: Parser<T>) : ArgumentType<Marshal<T>> {
    override fun parse(reader: StringReader?): Marshal<T> {
        parser.parse(reader!!)
    }
