...
open class FlagSet<T>(val toggles: MutableSet<Toggle>, val args: MutableSet<Argument<T>>) {
    ...
    fun getArgument(long: String) = args.firstOrNull { it.id.long == long }?.let { Result.success(it.value) }
        ?: Result.failure(Exception("Couldn't find argument $long, or it wasn't supplied."))

    fun getArgument(short: Char) = args.firstOrNull { it.id.short == short }?.let { Result.success(it.value) }
        ?: Result.failure(Exception("Couldn't find argument $short, or it wasn't supplied."))

    fun getFlagId(long: String) = (toggles.map { it.id } + args.map { it.id }).firstOrNull { it.long == long }
    fun getFlagId(short: Char) = (toggles.map { it.id } + args.map { it.id }).firstOrNull { it.short == short }

    fun without(long: String) = FlagSet(
        toggles.filter { it.id.long != long }.toMutableSet(),
        args.filter { it.id.long != long }.toMutableSet(),
    )

    fun without(vararg shorts: Char) = FlagSet(
